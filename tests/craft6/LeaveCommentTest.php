...
it('saves a comment on a specific field of a canonical entry', function () {
    $entry = seedEntry('posts', ['title' => 'My Post']);
...
it('persists a referenceId when the agent pins the comment to a CKEditor span', function () {
    $entry = seedEntry('posts', ['title' => 'Post']);
...
it('rejects a whole-field comment on a CKEditor field', function () {
    ...
    $ckeditorField = seedField('bodyContent', 'Body Content', \craft\ckeditor\Field::class);
    seedSection('articles', 'Articles', SectionType::Channel, [$ckeditorField]);

    $entry = seedEntry('articles', ['title' => 'Post']);
