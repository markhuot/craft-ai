...
use Craft;
// Eloquent model — exposes ::factory(). Used to seed a persisted volume.
use CraftCms\Cms\Asset\Models\Volume as VolumeModel;
use CraftCms\Cms\Field\PlainText;
// The craft\models\* aliases resolve to the Data DTOs (Section\Data\Section,
// etc.) in Craft 6 — and that's exactly the type the validators check with
// `instanceof craft\models\X`. The bound-phase tests construct bare DTOs, so
// they must use these aliases, not the Eloquent Models, to hit the bound path.
...
it('ExistingDraft accepts a real draftId and rejects unknown / wrong-type values', function () {
    seedSection('posts', 'Posts');
    $entry = seedEntry('posts', ['title' => 'source']);
...
it('ExistingVolume accepts handles and IDs and rejects unknown values', function () {
    $volume = VolumeModel::factory()->create(['name' => 'Photos', 'handle' => 'photos']);
...
it('ExistingEntryType scoped to a section rejects types from a sibling section', function () {
    $a = seedSection('secA', 'SectionA');
    $b = seedSection('secB', 'SectionB');

    $aType = $a->entryTypes()->first();
...
it('ExistingField accepts handles, UIDs, and IDs', function () {
    $field = seedField('bodyText', 'Body', PlainText::class);
...
it('ExistingField with uidOnly enforces UUID format and rejects handles', function () {
    $field = seedField('title2', 'Title2', PlainText::class);
...
it('AssetSettingsValidation requires a defaultUploadLocationSource when restrictLocation is false', function () {
    $volume = VolumeModel::factory()->create(['name' => 'Files', 'handle' => 'files']);
...
it('AssetSettingsValidation switches keys when restrictLocation is true', function () {
    $volume = VolumeModel::factory()->create(['name' => 'Restricted', 'handle' => 'restricted']);
